...
import { normalizeCityIds, normalizeRegionIds, QUEUE_NAMES } from "@amb/shared";
...
export async function filtersRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch<{ Params: { id: string } }>("/filters/:id", async (req, reply) => {
    ...
    const nextCities =
      data.cities ?? (data.regions ? normalizeCityIds(existing.cities, nextRegions) : existing.cities);
