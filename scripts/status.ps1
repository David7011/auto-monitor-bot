...
$backupHealth = Get-AmbBackupHealth $ProjectRoot
Write-Host 'Backup health (read-only, sanitized):'
$backupHealth | ConvertTo-Json -Depth 6
if ($backupHealth.status -eq 'FAIL') { $Failed = $true }
