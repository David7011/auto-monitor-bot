...
Финальный clean-run выявил различие структуры Redis ZIP на runner и локальном HP: установщик рекурсивно подтверждал бинарник, а acceptance ожидал его только в корне. Acceptance теперь использует тот же рекурсивный поиск. Внешний Google APT mirror один раз вернул Playwright индекс с `Hash Sum mismatch`; установка браузера получила три ограниченные попытки с обновлением APT metadata, без ослабления проверки целостности.
...
### P1-4: устранение общей точки отказа backup (реализация 2026-09-10)

Программная часть независимого зеркала усилена: copy выполняется через staging,
после копирования проверяется SHA-256, archive публикуется последним после sidecars,
а weekly restore drill при наличии `BACKUP_MIRROR_PATH` обязан восстановить именно
зеркальную копию. `local:status` проверяет отдельный том/UNC, наличие sidecar,
фактический SHA-256 и возраст не более 26 часов.

Физическая часть пока не завершена: аудит Windows видит только том `C:`, а
`BACKUP_MIRROR_PATH` не настроен. До подключения внешнего носителя или выделения
UNC/off-device storage этот пункт остаётся `IMPLEMENTED_NOT_ACTIVATED`; локальные
encrypted backups и restore drill продолжают работать как раньше.
