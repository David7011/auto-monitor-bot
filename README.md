...
Локальная высокоскоростная система мониторинга marketplace-объявлений с приоритетом OLX. Автомобильный мониторинг работает в production; переход к ноутбукам, смартфонам, ПК, видеокартам, консолям, электросамокатам и generic-поиску находится на этапе поэтапной проверки. Наличие profile в registry не означает завершённую live-приёмку категории. Проект сохраняет исходное наблюдение до фильтрации и как можно раньше отправляет первое сообщение в Telegram. Тяжёлое обогащение не входит в обычный критический путь первой отправки.
...
## Категории и безопасное расширение

- Старые фильтры и записи автоматически считаются `vehicle.car`; прежние автомобильные поля не удалены.
- Каждый OLX category/geography scope получает отдельный устойчивый discovery fingerprint, known tail, coverage и recovery. Состояния автомобилей и электроники не смешиваются.
- Несколько пользовательских фильтров одной категории объединяются в один discovery shard: один сетевой проход → несколько локальных проверок.
- У новой категории сначала рекомендуется включать `shadow`-режим: наблюдения, совпадения, parser/coverage метрики сохраняются, но Telegram не отправляется.
- Для новых non-car фильтров форма по умолчанию выбирает shadow. Запрет отправки сохраняется в `Listing.notificationMode`, поэтому переживает перезапуск и повторную обработку. Повышение до LIVE требует новой положительной оценки боевым фильтром.
- Ценовые границы электроники заданы в UAH; автомобильные границы сохраняют USD-семантику. Цена в неподдержанной валюте — UNKNOWN, а не неявный пересчёт.
- Если обязательная характеристика отсутствует на странице выдачи, результат становится `UNKNOWN`, а не ложным отказом. Политика по умолчанию `MAX_COVERAGE` отправляет provisional-карточку.
- `AUTO.RIA`, RST, Cars.ua и AutoMoto остаются автомобильными источниками; электроника не маршрутизируется к ним.
- HTTP 200 без ожидаемой OLX-структуры помечается `PARSER_DEGRADED`; такой проход не подтверждает coverage и не сдвигает доказанную границу.

Абсолютное отсутствие внешних пропусков невозможно обещать без официального event API площадки. Целевой контракт проекта — **ZERO SILENT INTERNAL LOSS**: принятое collector'ом объявление должно оставлять долговечное наблюдение и явный outcome либо восстанавливаемое состояние. Пройденные fault-тесты подтверждают конкретные сценарии, а не отсутствие всех возможных ошибок. Непроверенные части апгрейда и ограничения STRICT/generic перечислены в [контрольном отчёте](docs/MULTI_CATEGORY_CHECKPOINT_2026-09-07.md).
...
Telegram card/flash send lease сохраняет владельца по номеру попытки, продлевается каждые 15 секунд и проверяется перед HTTP send. Истёкший lease не возрождается, проигравшая попытка не сбрасывает состояние нового владельца. Redis gate выдаёт только текущий допуск и повторно проверяет cooldown после ожидания; это не отменяет принципиальную неоднозначность Telegram acceptance до локальной receipt. Подробности и приёмка: [Telegram lease/cooldown](./docs/TELEGRAM_LEASE_COOLDOWN_ACCEPTANCE_2026-09-13.md).
...
| OLX experiment | `OLX_EXPERIMENT_OWNER` | `cadence` | Единственный активный OLX-эксперимент: cadence, origin или none |
...
| OLX canary | `OLX_CADENCE_CANARY_P95_MIN_SAMPLES` | `30` | Минимум canary-проходов до решения по p95 |
| OLX canary | `OLX_CADENCE_CANARY_P99_MIN_SAMPLES` | `100` | Полных live-трасс до публикации достоверного p99 |
...
- `.env`, runtime и бэкапы закрыты ACL для текущего пользователя, `Administrators` и `SYSTEM`.
- Node загружается из официального источника и проверяется по закреплённому SHA-256.
...
Скрипт создаёт custom-format dump PostgreSQL, проверяет его через `pg_restore --list`, потоково шифрует встроенным `node:crypto` (AES-256-GCM и scrypt) и сохраняет `.ambbak`, SHA-256 и JSON metadata в `.runtime\backups`. Открытый dump удаляется. Пароль хранится только в защищённом локальном `.env` как `BACKUP_ENCRYPTION_PASSWORD` и передаётся криптографическому процессу через закрытый standard input, а не через аргументы процесса. GCM одновременно проверяет конфиденциальность и подлинность всего архива.
...
Панель остаётся недоступной из публичного интернета. Текущий режим — приватный TCP внутри зашифрованного Tailscale-туннеля.
